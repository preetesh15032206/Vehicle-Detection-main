...
def generate_demo_frames():
    ...
    while is_processing:
        ...
        draw_text(roi, "DEMO MODE - Simulated Vehicle Detection", 
                 pos=(10, roi.shape[0] - 30), color=(255, 255, 255), size=0.7)
        
        current_frame = frame
        time.sleep(0.033)  # ~30 FPS

def try_camera_capture():
    """Try to capture from camera, fallback to demo mode"""
    global current_frame, is_processing, tracking_data
    ...
    while is_processing:
        ret, frame = cap.read()
        if not ret:
            print("Failed to read from camera, switching to demo mode")
            cap.release()
            generate_demo_frames()
            return
